...
from AyiinRobot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("./AyiinRobot/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Ayiin Robot](https://t.me/YinzRobot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./AyiinRobot/resources/fonts"):
            os.remove(font_)
